...
class App(tk.Tk):
    ...
    def on_job_clicked(self, key):
        if key:
            # Autofocus only on the first selection of a new job
            if key != self.last_selected_job:
                self.do_autofocus()
    ...
    def _open_and_position_lb_for_current_job(self):
        ...
        if not ok: return
        # Removed delay and window management from here, as it's now handled by start_laser_flow
    ...
    def _do_position_bring_send_hotkey(self):
        ...
        time.sleep(2) # Give LightBurn a moment to fully appear, added an extra second
